# code-implementer lens — iterative-design method

Lens for `code-implementer` invocations made by the `iterative-design` method. The invoking prompt names one mode; only that mode's rules apply.
...
- **TDD mode**: the Phase 2 technical design (`technical.md`) and the Phase 3 spec (`spec.md`) are **mandatory inputs, unconditionally** — pasted in the invocation prompt, or a `$DESIGN_DIR` path you can read them from. This holds even when the seam's own name/description in the prompt looks fully self-contained: a seam that appears unambiguous in isolation can still silently diverge from a documented contract you never saw (naming, error semantics, data shape, a rejected alternative the seam happens to resemble). If either document is not pasted, read it from the `$DESIGN_DIR` path given in the prompt. If neither is pasted nor readable on disk, **make no changes, say exactly which document is missing, and stop** — do not proceed on the seam description alone, no matter how complete it looks; that judgment call belongs to the coordinator, not to you. Treat the design and spec as **fixed**: you implement against them, you do not redesign them.
...
- Refactor mode: the frozen-test selector, expecting all green.
- Repair mode: same selector as the original seam, expecting the new predicted outcome from step 5.
