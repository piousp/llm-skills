...
description: >
  ...
  pipeline de evaluadores (filologico, heuristico, APA, falacias, defectos
  epistemicos, hilo-conductor) usando subagentes. Pipeline state-machine con state.py read-only.
...
1. Informar al usuario explícitamente: "No tengo acceso a la herramienta `subagent`.
   Este skill requiere subagentes para delegar evaluación, consolidación y corrección."
...
- **`redactor`** — agente con capacidad de escritura. Recibe hallazgos en markdown
  y un archivo de trabajo, aplica las correcciones sugeridas. Usado en corrección
  y consolidación de hallazgos.
...
- **El coordinador lee los archivos y copia el contenido textual al prompt.**
  Nunca pasar rutas ni referencias a archivos. Usar `read` con `mode="full"`.
...
| **3 — Consolidate** | El `redactor` fusiona todos los `hallazgos-<eval>.md` en un único `hallazgos-consolidado.md` | redactor |
| **4 — Correct** | Una sola pasada del `redactor` con el consolidado, modifica `working.md` directamente | redactor |
...
Dentro de la Fase 3 (Consolidate):
- Una sola invocación al `redactor` que lee todos los hallazgos y escribe `hallazgos-consolidado.md`.
- El `redactor` devuelve "Work finished" o "FAILURE: <reason>".

Dentro de la Fase 4 (Correct):
- Una sola pasada del `redactor` con el consolidado.
...
| Campo | Descripción |
|-------|-------------|
| `phase` | Número de fase (1-4), `"done"`, o `"error"` |
...
nunca decide qué hacer — solo reporta lo que los archivos ya dicen. `derive_state()` es
100% read-only, no escribe en disco.
...
3. Loop:
   ...
   c. Si phase: "done" → presentar el resumen final (path al working file
      y status del consolidado) y detenerse. No hay handoff.
...
tiene cuatro campos: **Severidad**, **Ubicación**, **Problema**, **Corrección sugerida**.
...
- Si un subagente falla 2 veces:
  ...
  - Si es el `redactor` en consolidate: el coordinador escribe un consolidado mínimo directamente.
...
- **Saltarse la confirmación del usuario** — cada fase requiere confirmación
  explícita (la fase 4 tiene una sola confirmación, no una por hallazgo).
...
- **Pasar el consolidado al redactor por secciones** — el redactor recibe el archivo
  consolidado completo y hace una sola pasada.
