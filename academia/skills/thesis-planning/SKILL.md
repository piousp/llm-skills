...
a two-layer chapter drafting process (skeleton, then full draft, so structure
is confirmed before prose is written), a reviewer/tutor feedback loop with
lightweight versioning (short of git, by design), and reverse-outline
verification.

**[DO NOT] assume a permanent-notes layer or cross-chapter note network
exists; this skill does not implement one.** The skeleton layer (Phase 4a)
covers idea-synthesis per chapter, not across chapters.
...
- Delegate to `web-scout`: 2–4 parallel tasks, each covering one research axis (e.g.
  ...
  directly.
- **Always include a data/legal feasibility axis**: is the data the question needs
  actually accessible, and under what legal/access constraints? Skipping this axis
  produces a question that looks answerable on paper but isn't once Phase 1b tries
  to scope it.
...
- **Gate**: the user confirms or edits the skeleton before Phase 4b starts. This
  layer bridges "one paragraph of intent" to "full prose" before drafting begins.
