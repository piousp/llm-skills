...
| [`agent-prompting`](skills/agent-prompting/SKILL.md) | Guides how the coordinator writes and assembles delegation prompts for pi subagents (web-search, planning, implementation, review, meta-coordination): flow map, prompt anatomy, per-type templates in `references/`, delegate-vs-do criteria. Auto-triggered skill: loads when the coordinator is about to send a prompt to a subagent. |
